...
// ══════════════════════════════════════
// EDIÇÃO RÁPIDA DE ESTABELECIMENTO
// ══════════════════════════════════════
var qeIdx = -1;
...
function openQuickEdit(idx) {
  ...
  var r = allData[idx];
  if (!r) return;
  document.getElementById('qe-store-label').textContent = r.nome + ' — ' + r.mes;
  document.getElementById('qe-nome').value     = r.nome || '';
  document.getElementById('qe-mes').value      = r.mes  || 'Janeiro';
  document.getElementById('qe-exec').value     = r.executivo || '';
  document.getElementById('qe-hub').value      = r.informado || 0;
  document.getElementById('qe-real').value     = r.real || 0;
  document.getElementById('qe-realizado').value= r.momento || 0;
  document.getElementById('qe-mrr').value      = r.mrr || 0;
  document.getElementById('qe-os').value       = r.oneshot || 0;
  document.getElementById('quick-edit-overlay').classList.add('open');
}

function closeQuickEdit() {
  document.getElementById('quick-edit-overlay').classList.remove('open');
  qeIdx = -1;
}

function saveQuickEdit() {
  if (qeIdx < 0 || !allData[qeIdx]) return;
  var r = allData[qeIdx];
  r.nome      = document.getElementById('qe-nome').value.trim() || r.nome;
  r.mes       = document.getElementById('qe-mes').value;
  r.executivo = document.getElementById('qe-exec').value;
  r.informado = parseFloat(document.getElementById('qe-hub').value)       || 0;
  r.real      = parseFloat(document.getElementById('qe-real').value)      || 0;
  r.momento   = parseFloat(document.getElementById('qe-realizado').value) || 0;
  r.mrr       = parseFloat(document.getElementById('qe-mrr').value)       || 0;
  r.oneshot   = parseFloat(document.getElementById('qe-os').value)        || 0;
  salvarCasaSupabase(r);
  closeQuickEdit();
  refreshAllViews();
  toast('✅ Estabelecimento salvo!');
}

function deleteFromQuickEdit() {
  if (qeIdx < 0 || !allData[qeIdx]) return;
  var r = allData[qeIdx];
  if (!confirm('Remover "' + r.nome + '"?')) return;
  deletarCasaSupabase(r.nome, r.mes);
  allData.splice(qeIdx, 1);
  closeQuickEdit();
  refreshAllViews();
  buildMonthChips([...new Set(allData.map(function(d){return d.mes;}))].sort(function(a,b){return MONTHS_ORDER.indexOf(a)-MONTHS_ORDER.indexOf(b);}));
  toast('🗑 Casa removida!');
}
...
function getFiltered() {
  ...
  if(fe) rows = rows.filter(r=>(r.executivo||'').toUpperCase()===fe.toUpperCase());
...
// ── EVOLUÇÃO ANUAL
function renderEvo() {
  var months = [...new Set(allData.map(d=>d.mes))].sort((a,b)=>MONTHS_ORDER.indexOf(a)-MONTHS_ORDER.indexOf(b));
...
function renderPending() {
  ...
  document.getElementById('pending-body').innerHTML = pending.map(r => {
    var p = pctReal(r);
    var nameEsc = r.nome.replace(/'/g, "\'");
    var mot = motivos[r.mes+'|'+r.nome];
    var temMotivo = mot && mot.text;
    var rowBgPend = temMotivo ? 'background:#f0fdf4' : '';
    return `<tr style="${rowBgPend}">
      ...
      <td>${temMotivo
        ? `<span style="font-size:11px;font-weight:600;background:#dcfce7;color:#16a34a;padding:2px 8px;border-radius:99px">✅ Justificado</span>
           <div style="font-size:11px;color:#475569;margin-top:3px;max-width:200px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="${mot.text}">${mot.text}</div>
           <div style="font-size:10px;color:#94a3b8">${mot.exec||''} · ${mot.cat||''}</div>`
        : ''}</td>
      <td><button class="btn-edit" onclick="openMotivoMes('${nameEsc}','${r.mes}',${Math.round(p*100)})">
        <svg viewBox="0 0 24 24"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
        ${temMotivo ? 'Editar' : 'Registrar'}
      </button></td>
    </tr>`;
...
function switchTab(id, btn) {
  ...
  // Hide global month bar on tabs that have their own month filter
  var globalBar = document.getElementById('global-month-bar');
  if (globalBar) globalBar.style.display = (id==='backlog'||id==='motivos') ? 'none' : '';
